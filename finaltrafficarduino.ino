...
volatile bool waitingForInput = false; // Flag to indicate waiting for user input
...
void loop() {
  ...
  if (interruptFlag && !receivedInterrupt) {
    ...
    waitingForInput = true; // Set waiting for input flag
  }

  if (waitingForInput && newDataReceived) {
    ...
    if (receivedValue >= 1 && receivedValue <= numLeds) {
      ...
      if (receivedValue >= 1 && receivedValue <= 4) {
    // Turn on the corresponding LED for 25 seconds
    digitalWrite(ledPins[receivedValue - 1], HIGH);
    delay(25000); // 25 seconds delay
    digitalWrite(ledPins[receivedValue - 1], LOW);
  }
  }
      ...
      // Loop until the 25 seconds duration is over or new data is received
      while (millis() - startTime < ledDuration && !newDataReceived) {
      ...
      newDataReceived = false; // Reset new data received flag
